#!/usr/bin/env python3
"""
Reginald Daily Cartoon (No Framebuffer Version)
Generates a single-panel Reginald illustration for web upload.
Runs via cron. Saves to ~/.cache/reginald-daily/
"""

import os, sys, time, json, base64, urllib.request, random
...
def main():
    print(f"Reginald Daily (No FB): {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    
    success=generate_reginald_art()
    if success:
        state={"date":datetime.now().strftime("%Y-%m-%d"),"timestamp":time.time()}
        ...
        # Generate a Reginald quote for today
        quote = get_reginald_quote()
        print(f"Quote: {quote}")
        
        print("Done — ready for web upload")
        return 0
    else:
