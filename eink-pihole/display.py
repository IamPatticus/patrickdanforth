#!/usr/bin/env python3
"""Render Pi-hole stats on Waveshare 2.13" E-Ink HAT V4."""
import os
import sys
import time
import traceback
...
def render(summary, error=None):
    ...
    draw.text((MARGIN, 2), "PI-HOLE STATUS", font=font_bold, fill=255)

    y = 22

    if error:
        draw.text((MARGIN, y), "Error:", font=font, fill=0)
        y += LINE_H
        for line in wrap_text(error, font_small, WIDTH - MARGIN * 2):
            draw.text((MARGIN, y), line, font=font_small, fill=0)
            y += LINE_H - 1
            if y > HEIGHT - 10:
                break
        return image
    ...
    # Row 1
    draw.text((MARGIN, y), f"Queries: {format_number(total)}", font=font, fill=0)
    draw.text((WIDTH // 2, y), f"Blocked: {format_number(blocked)}", font=font, fill=0)
    y += LINE_H + 2

    # Row 2
    draw.text((MARGIN, y), f"Blocked %: {percent}%", font=font, fill=0)
    draw.text((WIDTH // 2, y), f"Clients: {unique_clients}", font=font, fill=0)
    y += LINE_H + 2
    ...
    # Blocklist count
    draw.text((MARGIN, y), f"Blocklist: {format_number(blocklist_domains)} domains", font=font_small, fill=0)
    y += LINE_H

    # Timestamp
    now = time.strftime("%Y-%m-%d %H:%M")
    draw.text((MARGIN, HEIGHT - 12), f"Updated: {now}", font=font_small, fill=0)
...
def update_display(image):
    epd = epd2in13_V4.EPD()
    epd.init()
    # Clear to white first, then draw content
...
def main():
    ...
    image = render(summary, error)
    try:
        update_display(image)
        print("Display updated.")
    except Exception:
        # Save to file for debugging if e-ink fails
        debug_path = Path.home() / "eink-pihole" / "debug.png"
